...
class CardDeck extends StatefulWidget {
  final List<SingleCard> cardDeck;
...
class _CardDeckState extends State<CardDeck> with TickerProviderStateMixin {
  final reviewCards = <SingleCard>[];
  final masteredCards = <SingleCard>[];
    ...
    ..addListener((){
      setState(() {
        ...
        if (_scrollPercent == 1.0) {
          ...
          masteredCards.add(widget.cardDeck[index]);
          widget.cardDeck.removeAt(index);
          index -= 1;
          _scrollPercent = 0.0;
        }
  ...
  Widget _activeCard(){
    return FractionalTranslation(
      ...
      child: Transform(
        ...
        child: widget.cardDeck[index],
      ),
    );
  }

  Widget _inactiveCards(List<SingleCard> cards, int index) {
    if (index == 0) {
      return Container(
        margin: EdgeInsets.fromLTRB(12.0, (index + 1) * 20.0, 12.0, 12.0),
        child: cards[0]
      );
    } else {
      return Stack (
        ...
        children: [
          ...
          Container (
            ...
            child: GestureDetector(
              ...
              onHorizontalDragEnd: _handleSwipeEnd,
              onTap: () {Tts.speak(widget.cardDeck[index].name);},
              //onLongPress: () => Tts.speak(widget.cardDeck[index].name),
              child: _activeCard(),
            )
          )
        ]
